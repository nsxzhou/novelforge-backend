...
- 已接入项目、资产、章节与对话微调 API：
  ...
  - `PUT /api/v1/projects/:projectID/prompts/:action`
  - `GET /api/v1/llm/providers`
  - `GET /api/v1/llm/providers/sse`
...
| 提示词重写 | `domain/prompt`       | `PromptOverride`              |
| 大模型服务 | `domain/llmprovider`  | `LLMProvider`                 |
...
- 一致性守卫（冲突检测与修复建议）
- 批量章节生成与质量评分
- 导出中心（Markdown/TXT/EPUB）
- 成本看板（可视化面板，V1 仅保留指标埋点）
...
- `migrations/000003_project_prompt_overrides.up.sql`
- `migrations/000003_project_prompt_overrides.down.sql`
- `migrations/000004_llm_providers.up.sql`
- `migrations/000004_llm_providers.down.sql`
